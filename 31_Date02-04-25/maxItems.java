...
public class maxItems{
    public static int solveN(int[] itemHeights, int[] roomHeights){
        Arrays.sort(itemHeights);
        int count = 0;
        int n = itemHeights.length;
        int k = roomHeights.length;
        boolean there[] = new boolean[k];
        for(int i=0;i<n;i++){
            int ind = -1;
            for(int j=0;j<k;j++){
                if(!there[j] && itemHeights[i]<=roomHeights[j]){
                    ind = j;
                }
                else{
                    break;
    ...
    public static int solve(int[] itemHeights, int[] roomHeights){
        Arrays.sort(itemHeights);
        int count = 0;
        int n = itemHeights.length;
        int k = roomHeights.length;
        ...
        int right = k-1;
        for(int i=1;i<k;i++){
            roomHeights[i] = Math.min(roomHeights[i], roomHeights[i-1]);
        }
        while(left<n && right>=0){
            if(itemHeights[left]<=roomHeights[right]){
                left++;
                count++;
            }
            right--;
    ...
    public static void main(String[] args){
        ...
        int n = s.nextInt();
        int k = s.nextInt();
        int itemHeights[] = new int[n];
        int roomHeights[] = new int[k];
        for(int i=0;i<n;i++){
            itemHeights[i] = s.nextInt();
        }
        for(int i=0;i<k;i++){
            roomHeights[i] = s.nextInt();
        }
        System.out.println(solve(itemHeights,roomHeights));
    }
}