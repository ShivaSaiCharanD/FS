...
public class treasureVault{
    public static int score(int[] arr,int x,int f,int start,int end){
        ...
        for(Map.Entry<Integer,Integer> entry : freq.entrySet()){
            int priority = entry.getValue() * ((int)Math.pow(entry.getKey(),f));
            q.offer(new int[]{entry.getKey(),priority});
        }
        int sum=0;
        for(int i=0;i<x;i++){
            int[] temp = q.poll();
            sum+=temp[0]*freq.get(temp[0]);
        }
        return sum;
    }
    public static void main(String[] args){
        ...
        for(int i=0;i<n-k;i++){
            res.add(score(arr,x,f,i,i+k));
        }
        res.add(score(arr,x,f,n-k,n));
